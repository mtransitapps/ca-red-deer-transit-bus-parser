...
JAVA_FILES_DIR="src/org/mtransit/parser/ca_red_deer_transit_bus";
JAVA_STOPS_FILE="$JAVA_FILES_DIR/Stops.java";
echo ">> Pre Parsing > Set Java stops file...";
> $JAVA_STOPS_FILE; # empty file
echo "package org.mtransit.parser.ca_red_deer_transit_bus;" >> $JAVA_STOPS_FILE;
...
awk -F "\"*,\"*" '/^\s*$/ {next;} { print "		allStops.put(\"" $2 "\", \"" $1"\"); // " $3}' input/gtfs/stops.txt >> $JAVA_STOPS_FILE;
checkResult $? false;
